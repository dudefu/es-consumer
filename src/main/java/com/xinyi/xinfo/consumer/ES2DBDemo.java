...
import com.xinyi.xinfo.conf.Config;
import com.xinyi.xinfo.utils.MapKeyToLower;
...
//        esDemo.directExport("127.0.0.1", "9200", "kibana_sample_data_logs", "kibana_sample_data_logs");
//        esDemo.directExport("127.0.0.1", "9200", "demo", "demo");
//        esDemo.directExport("127.0.0.1", "9200", "kibana_sample_data_ecommerce", "kibana_sample_data_ecommerce_1577099395385");
//        esDemo.directExport("127.0.0.1", "9200", "kibana_sample_data_flights", "kibana_sample_data_flights");behavior
        esDemo.directExport("10.200.152.77", "29200", "behavior", "behavior");
//		esDemo.exportData();
     ...
     * 采用原始方法导入数据
     */
    public void directExport(String server, String port, String indexName, String targetTableName) {
        ...
        String dsl2ndSqlFile = "dsl2ndSqlFile-1.xml";
        ...
        final BatchHandler<Map> batchHandler = new BatchHandler<Map>() {
            ...
            public void handler(PreparedStatement stmt, Map esrecord, int i) throws SQLException {
                esrecord = MapKeyToLower.getMapKeyToLower(esrecord);
//				stmt.setInt(1, (Integer) esrecord.get("score"));
//				stmt.setString(2, (String) esrecord.get("description"));
...
//                System.out.println(i);
                for (int j = 0; j < fieldsNew.size(); j++) {
                    ...
                    Object record = esrecord.get(field) ;
                    if(!"".equals(record) && record != null){
                        if (record instanceof Integer) {
                            stmt.setInt(j + 1, (Integer) record);
                        }else {
                            stmt.setString(j + 1,  record.toString());
                        }
                    }else {
                        stmt.setString(j + 1,  (String)record);
                    }
                }
            }
        };
...
//					 .setSliceQuery(true)
//				     .setSliceSize(5)
                .setQueryUrl("province/_search"); //查询索引表demo中的文档数据

        exportBuilder.setBatchHandler(new BatchHandler<Map>() { //重要每条文档记录交个这个回调函数处理
            @Override
            public void handler(PreparedStatement stmt, Map esrecord, int i) throws SQLException {
                stmt.setInt(1, (Integer) esrecord.get("id"));
                stmt.setString(2, (String) esrecord.get("name"));
                stmt.setString(3, (String) esrecord.get("description"));
                stmt.setInt(4, (Integer) esrecord.get("score")); //将当期文档的字段设置到db批处理语句中
            }
        })
        //				//添加dsl中需要用到的参数及参数值
//				.addParam("var1","v1")
